...
Twee deltakolommen werken op alle SQL-/databasebronnen: **SQL Server, Azure SQL Database, MySQL, PostgreSQL, Oracle, DB2, Sybase, Snowflake en OneStream** (de engine gebruikt een ANSI-`COALESCE`, dus ook MySQL; de Snowflake-pipeline voert de gegenereerde query inclusief delta-WHERE uit als bronquery). Daarnaast ondersteunen ook **Salesforce, SAP SAC en AFAS** twee deltakolommen (sinds v1.56).
:::
