...
4. **Per tabel laden (ForEach).** De `ForEach`-activiteit "Load data" draait **parallel** (`batchCount` 5). Per tabel gebeurt het volgende:
   ...
   - **STAGE → HIS.** `Load DWH` roept **`[LoadManagement].[spLoadDWH]`** aan, dat — na een arbitragecheck tegen een lopende archiveringsrun — **`[LoadManagement].[spHIS_InsertAndUpdate]`** aanroept. Deze procedure doet de echte, set-based **SCD2-merge** van `STAGE` naar de history-laag (zie [SCD2 & hashing](#scd2-de-historie-laag) hieronder).
...
`spLoadDWH` doet eerst een **arbitragecheck**: staat er voor dit target nog een `RUNNING` archiveringsrun (een `LoadLog`-rij met LoadType `ARCHIVING`), dan laat hij de load bewust falen — laden en archiveren mogen dezelfde tabel niet tegelijk raken. Daarna roept hij `spHIS_InsertAndUpdate` aan (de oude aanroep van `spUpdateETL_EndDate` staat uitgecommentarieerd — het afsluiten van versies gebeurt nu binnen `spHIS_InsertAndUpdate` zelf). Wat de merge precies doet hangt af van het [load type](./load-types.md), maar het mechanisme is voor alle types hetzelfde **SCD2**-model (Slowly Changing Dimension type 2):
