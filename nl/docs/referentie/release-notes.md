...
- **De actief-instelling van een kolom of tabel werkt nu door in de loads.** Zet je een kolom of
  tabel op inactief, dan wordt die vanaf 1.56 ook daadwerkelijk overgeslagen bij het laden.
  Controleer daarom vóór de upgrade wat er bij jou op inactief staat, zodat je selectie klopt met wat
  je geladen wilt hebben.
  → [Acties per bron](../frontend/data-sources.md#acties-per-bron-afhankelijk-van-brontype)
- **Een kolom verwijderen ruimt hem nu ook fysiek op.** Haal je een kolom uit de dictionary — de rij
  weghalen, niet alleen op inactief zetten — dan verwijdert Yres hem voortaan ook uit de HIS-tabel,
  **inclusief de historische waarden die erin staan**. Voorheen bleef de kolom in de database staan.
  Deze actie is niet terug te draaien; wil je de historie bewaren, zet de kolom dan op **inactief** in
  plaats van hem te verwijderen.
- **Uniciteit op de kolomadministratie.** De tabellen achter de dictionary krijgen bij de upgrade een
...
- **Bijna 1700 gedragscontroles op het dataplatform.** De meegeleverde testsuite is uitgebreid van
  ...
  - **Delta-loads rond de zomertijdovergang** verwerken de twee overgangsnachten nu correct.
  - **Het promotiemechanisme** neemt bij een tabeltype de volledige definitie mee naar de volgende
    omgeving, inclusief kolombreedtes en standaardwaarden.
  - **Gedeeltelijke wijzigingen** in de webapp laten de velden die je niet aanraakt ongemoeid.
  - **Een onverwerkbaar item** stopt de rest van een verwerking niet meer; die loopt door en het
    probleemgeval wordt gelogd.
  - **Het auditspoor van databasewijzigingen** blijft ook na een onderbroken opruimactie actief.
