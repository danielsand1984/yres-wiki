...
[**OData OAuth**](odata-oauth.md). A few helper sources (CBS, Tweede Kamer, Topdesk, Dynamics 365,
Intune) are also stored as type `OData` under the hood, but have their own input fields;
document those on their own pages. **Microsoft Graph** does not belong in this list: it is stored as
an `ODataoAuth` source.
:::
