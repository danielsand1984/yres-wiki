...
| 3 | **Install change** — import and install the change on the target environment for the first time, including the ADF publish | ☰ → **prd** submenu · released change | endpoint `/changes/install-and-import`: `[Change].[spImport]` + `[Change].[spInstall]` + `publish-datafactory` |
| 4 | **Reimport / Reinstall change** — afterwards: re-import (DWH only) or re-install (without import) | ☰ → **prd** submenu · installed change | `[Change].[spImport]` or `[Change].[spInstall]` + `publish-datafactory` |
...
7. **Actions (☰)** — the per-row actions menu; it contains **Update**, **Delete**, a **per-environment submenu** (release / install / reimport / reinstall), **View dependencies** and **Logs** (see [Releasing](#releasing-a-change) and [Installing](#installing-a-change)).
...
Releasing, installing, (re)importing and (re)installing happen via the **actions menu (☰)** that every change row in the table has. In addition, the Projects sidebar also holds the separate **Release change** (`/changes/release`) and **Install change** (`/changes/install`) screens; the actions menu bundles the same actions per change row. Which items the menu shows depends on the change's status and on the environment.
...
- **A per-environment submenu** (e.g. **dev ▸**, **prd ▸**):
  ...
  - On a **target environment** (e.g. **prd**), a **released** change first offers **Install change** — this imports and installs the change and publishes the ADF factory (endpoint `/changes/install-and-import`). Only after the change has been installed on that environment do **Reimport change** (re-import only, `[Change].[spImport]`) and **Reinstall change** (re-install without an import: `[Change].[spInstall]` + the `publish-datafactory` pipeline) appear.
...
- Yres first runs a **dependency check** and **blocks** releasing when this change itself depends on changes that have **not yet been released**; the error message names the change(s) this change depends on.
...
3. Choose the action that fits the situation:
   - **Install change** — the first time on this environment. Imports the change and project data (including dependencies and content), **installs** the change with `[Change].[spInstall]` **and publishes the ADF factory** by running the Azure DevOps pipeline `publish-datafactory`, so that new data-source pipelines land in the target factory as well (endpoint `/changes/install-and-import`).
   - **Reimport change** — only visible after the change has been installed on this environment. Calls only `[Change].[spImport]`: re-fetches the change and project data. This is **DWH only** and does **not** publish the ADF factory.
   - **Reinstall change** — likewise only visible after installation. Re-installs with `[Change].[spInstall]` and publishes the ADF factory, but does **not** import.
4. Follow the progress in the **Logs** (see below) and in the change's **Status overview**.

:::note Versions must match
The first **Install change** (import-and-install) first checks whether the **DWH versions** of the source and target environment match. If they differ, you get *"Environment versions do not match, please update"* and you must first update the environment via [Update environments](./admin.md). **Reimport** and **Reinstall** do not perform this version check.
:::
...
:::info Actions menu (☰) plus separate screens
Releasing, installing, (re)importing and (re)installing happen via the change's **actions menu (☰)** on the **Changes** screen: on **dev** an open change offers **release change**, on a target environment a released change offers **Install change** and — after installation — **Reimport change** and **Reinstall change**. In addition, the Projects sidebar also holds the separate *Release change* (`/changes/release`) and *Install change* (`/changes/install`) screens.
:::
